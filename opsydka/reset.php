...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["new_password"]))){
        $new_password_err = "Εισαγάγετε τον νέο κωδικό πρόσβασης.";     
    } elseif(strlen(trim($_POST["new_password"])) < 6){
        $new_password_err = "Ο κωδικός πρόσβασης πρέπει να έχει τουλάχιστον 6 χαρακτήρες.";
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        $confirm_password_err = "Επιβεβαιώστε τον κωδικό πρόσβασης.";
    } else{
        $confirm_password = trim($_POST["confirm_password"]);
        if(empty($new_password_err) && ($new_password != $confirm_password)){
            $confirm_password_err = "Ο κωδικός πρόσβασης δεν ταιριάζει.";
        }
    }
    ...
    if(empty($new_password_err) && empty($confirm_password_err)){
        ...
        $sql = "UPDATE erecyclebin_opsydka SET password = ? WHERE id = ?";
